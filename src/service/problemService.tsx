...
import { GetProblemsParams, GetProblemsResponse, IProblem } from "../types/component.types";
...
const getAllProblems = async ({
  ...
  const addProblems = async (problems:IProblem)=>{
    try {
      console.log("dfv")
      const response=await axiosInstance.post(`${ADMIN_API}/addproblems`,problems)
      return response.data
    } catch (error) {
      const err = error as ApiError;
      return {
  ...
  const updateProblem = async (problemId: string, data: IProblem) => {
    try {
      const response = await axiosInstance.put(`${ADMIN_API}/updateproblem/${problemId}`, data);
      return response.data;
    } catch (error) {
      const err = error as ApiError;
      return {
  ...
  const getProblems = async (params: { visible?: boolean; query?: string; difficulty:string }) => {
    try {
 ...
 const  getSingleProblem=async (problemId: string) => {
    try {
      const response = await axiosInstance.get(`${USER_API}/singleproblem/${problemId}`);
      console.log("problem response",response)
      return response;
    } catch (error) {
      const err = error as ApiError;
  ...
  const toggleProblemStatus = async (problemId: string) => {
    try {
      const response = await axiosInstance.patch(`${ADMIN_API}/problems/${problemId}`);
      return response.data;
    } catch (error: unknown) {
      const err = error as ApiError;
      return {
